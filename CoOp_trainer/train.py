...
def main():
    ...
    # load model
    _, preprocess = clip.load(cfg.backbone)
    ...
    for epoch in range(0, epochs):
        ...
        if epoch%5==0:
          ...
          if new_record is None or avg_acc > new_record:
              new_record = avg_acc
              count = 0
              file_name = f'{cfg.choose_feature}_prompt_learner.pth'
              file_path = os.path.join(cfg.save_dir, file_name)
              if os.path.isfile(file_path):
                  os.remove(file_path)
              torch.save(prompt_learner.state_dict(), file_path)
          else:
              count+=1
          print(f'{epoch + 1} epochs : accuracy : {avg_acc}')

          if count == cfg.fail_update_new_record:
              print(f'Failed to break new record {cfg.fail_update_new_record} times.\nTrainig ends. ')
              break
